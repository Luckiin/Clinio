    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Gestão do corpo clínico e agenda padrão</p>
        </div>
            ...
            {carregando ? (
              ...
              medicos.map(med => (
                <tr key={med.id}>
                  <td>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {med.especialidade && <p className="text-xs text-slate-500">{med.especialidade} • CRM {med.crm}</p>}
                      </div>
                    </div>
                  </td>
                  <td>
                  ...
                  <td>
                    {med.ativo ? (
                      <span className="badge bg-green-100 text-green-700 border border-green-200"><CheckCircle2 className="w-3 h-3 mr-1" /> Ativo</span>
                    ) : (
                      <span className="badge bg-slate-100 text-slate-500 border border-slate-200"><XCircle className="w-3 h-3 mr-1" /> Inativo</span>
                    )}
                  </td>
      ...
      {modalAberto && medicoEmEdicao && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-modal w-full max-w-xl overflow-hidden animate-fade-in transition-all">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-lg font-bold text-slate-800">{medicoEmEdicao.id ? 'Editar Médico' : 'Novo Médico'}</h2>
              <button
            ...
            <form onSubmit={salvarMedico} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2 items-center">
                    <input
                      ...
                      required
                    />
                    <span className="text-sm font-mono text-slate-500 uppercase">{medicoEmEdicao.cor_agenda}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                />
                <label htmlFor="ativo" className="text-sm text-slate-700 font-medium">Médico Ativo no Sistema</label>
              </div>
